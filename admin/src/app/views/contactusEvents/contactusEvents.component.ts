...
@Component({
  templateUrl: 'contactusEvents.component.html'
})
export class ContactusEventsComponent implements OnInit {
  ...
  getAllDashboardEvents() {
    ...
    this.dashboardService.getAllContactUsEvents().subscribe((data: {}) => {
  ...
  filter(){
    ...
    this.dashboardService.getAllContactUsByFilter(condition).subscribe((data: {}) => {
      this.dataSource = new MatTableDataSource(data['Group']);
      this.Entries = data['result'];
      this.dataSource.paginator = this.paginator;
      this.dataSource.sort = this.sort;
  ...
  exportToExcel() {
    ...
    new Angular5Csv(localArray, 'ContactUs', options);
  }
}
