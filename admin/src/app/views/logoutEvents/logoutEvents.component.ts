...
@Component({
  templateUrl: 'logoutEvents.component.html'
})
export class LogoutEventsComponent implements OnInit {
  ...
  displayedColumns: string[] = ['StudyCode' ,'UserInitial' ,  'Event' , 'DateTime'];
  ...
  getAllDashboardEvents() {
    ...
    this.dashboardService.getAllLogoutEvents().subscribe((data: {}) => {
  ...
  filter(){
    ...
    this.dashboardService.getAllLogoutByFilter(condition).subscribe((data: {}) => {
      this.dataSource = new MatTableDataSource(data['Group']);
      this.Entries = data['result'];
      this.dataSource.paginator = this.paginator;
      this.dataSource.sort = this.sort;
